...
'''
Copy of .\detailed-tests\test_openai_models_reasoning.py.
Tests OpenAI models on a small sample of SuperGM theme (difficult) chess puzzles.
Meant to demonstrate the extreme increase in response time for using GPT-5's Reasoning effort=medium,high
'''
    ...
    def load_puzzles_from_file(self, file_path: str, max_puzzles: int = None, random_sample: bool = True) -> List[List[Dict]]:
        ...
        while i < len(lines):
            ...
            if line == '<puzzle-start/>':
                ...
                # Handle both formats: with and without "Legal moves:" line
                if i + 1 < len(lines) and lines[i + 1].startswith('Legal moves:'):
                    legal_moves = lines[i + 1].replace('Legal moves:', '').strip()
                    skip_lines = 1
                    if i + 2 < len(lines) and lines[i + 2].startswith('Best move:'):
                        best_move = lines[i + 2].replace('Best move:', '').strip()
                        skip_lines = 2
                elif i + 1 < len(lines) and lines[i + 1].startswith('Best move:'):
                    # Format without "Legal moves:" line
                    best_move = lines[i + 1].replace('Best move:', '').strip()
                    skip_lines = 1
                ...
                i += skip_lines  # Move past the lines we've processed
            ...
            try:
                ...
                if model.startswith("gpt-5"):
                    ...
                    if reasoning_effort in ["high", "medium"]:
                        max_output_tokens = 30000  # Much larger token budget for extended reasoning
                    else:
                        max_output_tokens = 30  # Minimal tokens for quick responses

                    # Create reasoning config with proper typing
                    from typing import cast
                    from typing import Literal
                    reasoning_config = Reasoning(effort=cast(Literal['minimal', 'low', 'medium', 'high'], reasoning_effort))
...
def main():
    ...
    for puzzle_type, filename in puzzle_files.items():
        ...
        puzzles = tester.load_puzzles_from_file(file_path, max_puzzles=10, random_sample=True)
    ...
    output_file = os.path.join(project_root, "openai_models_speeds.json")
    ...
    # Print summary
    print(f"\n{'='*120}")
    print("SUMMARY - All Models and Puzzle Types")
    print(f"{'='*120}")
    print(f"{'Puzzle Type':<20} {'Model':<30} {'Move Acc':<20} {'Puzzle Acc':<15} {'Avg Time/Move':<15} {'Tokens':<15}")
    print(f"{'-'*120}")
    ...
    for puzzle_type in puzzle_files.keys():
        ...
        for model_key in sorted(all_results[puzzle_type].keys()):
            ...
            print(f"{puzzle_type:<20} {display_name:<30} {move_acc:<20} {puzzle_acc:<15} {avg_time:<15} {total_tokens}")
