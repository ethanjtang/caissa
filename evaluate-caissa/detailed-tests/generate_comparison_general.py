...
'''
Uses JSON result files from all chess models (GPTs, Stockfish, Caissa) to generate graphics.
'''
...
def generate_accuracy_bar_chart(overall_stats: Dict[str, Dict], output_dir: str):
    """Generate bar chart for overall model accuracy (puzzle accuracy)."""

    # Sort by puzzle accuracy (descending)
    sorted_models = sorted(overall_stats.items(), key=lambda x: x[1]['puzzle_accuracy'], reverse=True)

    model_names = [name for name, _ in sorted_models]
    accuracies = [stats['puzzle_accuracy'] for _, stats in sorted_models]
    ...
    ax.set_title('Overall Model Performance - Puzzle Accuracy Across All Chess Puzzles', fontsize=18, fontweight='bold', pad=30)
    ...
    bars = ax.bar(range(len(model_names)), accuracies, color=bar_colors, edgecolor='black', linewidth=1.2, zorder=3)
    ...
    # Save the figure
    output_path = os.path.join(output_dir, 'overall_model_accuracy.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Saved accuracy bar chart to: {output_path}")
    plt.close()


def generate_sanity_bar_chart(overall_stats: Dict[str, Dict], output_dir: str):
    """Generate bar chart for overall model reliability (sanity rate = valid moves / total moves)."""

    # Sort by sanity rate (descending)
    sorted_models = sorted(overall_stats.items(), key=lambda x: x[1]['sanity_rate'], reverse=True)

    model_names = [name for name, _ in sorted_models]
    sanity_rates = [stats['sanity_rate'] for _, stats in sorted_models]
    ...
    # Customize the chart
    ax.set_xlabel('Model', fontsize=16, fontweight='bold')
    ax.set_ylabel('Sanity Rate (Valid Moves / Total Moves) (%)', fontsize=16, fontweight='bold')
    ax.set_title('Overall Model Reliability - Sanity Rate Across All Chess Puzzles', fontsize=18, fontweight='bold', pad=30)
    ax.set_xticks(range(len(model_names)))
    ax.set_xticklabels(model_names, rotation=45, ha='right', fontsize=13)
    ...
    bars = ax.bar(range(len(model_names)), sanity_rates, color=bar_colors, edgecolor='black', linewidth=1.2, zorder=3)

    # Add value labels on bars
    for i, (bar, rate) in enumerate(zip(bars, sanity_rates)):
        height = bar.get_height()
        ax.text(bar.get_x() + bar.get_width()/2., height + 1,
                f'{rate:.1f}%',
                ha='center', va='bottom', fontsize=13, fontweight='bold', zorder=4)
    ...
    # Save the figure
    output_path = os.path.join(output_dir, 'overall_model_sanity_rate.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Saved sanity rate bar chart to: {output_path}")
    plt.close()


def generate_response_time_bar_chart(overall_stats: Dict[str, Dict], output_dir: str):
    """Generate bar chart for overall model response times (avg time per move)."""
    ...
    ax.set_title('Overall Model Response Times Across All Chess Puzzles', fontsize=18, fontweight='bold', pad=20)
    ax.set_xticks(range(len(model_names)))
    ax.set_xticklabels(model_names, rotation=45, ha='right', fontsize=13)

    # Set up grid with major ticks every 0.1s and minor ticks every 0.05s
    ...
    # Save the figure
    output_path = os.path.join(output_dir, 'overall_model_response_times.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Saved response time bar chart to: {output_path}")
    plt.close()
...
def main():
    ...
    # Script is now in the main directory with JSON files
    ...
    # Generate all three bar charts
    generate_accuracy_bar_chart(overall_stats, graphs_dir)
    generate_sanity_bar_chart(overall_stats, graphs_dir)
